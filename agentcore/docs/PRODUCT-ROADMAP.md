...
Current live evidence covers Cognito login, deterministic control mutations, scripted multi-turn Memory, Code Interpreter, usage queries, one sticky Runtime session, the fixed model dropdown, capped Runtime turns for all five Bedrock models after Marketplace activation, a capped Gemini turn through AgentCore Identity, desktop/mobile conversation reopen after switching and reload, revisioned agent edits and tool grants, Code Interpreter grant/rotation/revocation, and a read-only real GitHub /user call. Disposable-stack tests also exercised real Gateway Web Search, default-deny Cedar policy, managed Browser OS actions/screenshot, cited search results, tool usage rows, and reopened citation links with scripted inference only. A JSON metering receipt is saved alongside each Memory turn. It does **not** cover all Cognito roles, cross-store reconciliation, accurate total AWS cost, Gemini managed-tool calls, Browser DOM/live view, skills, or an exhaustive visual/accessibility pass. The production model list is checked-in JSON, not an admin table or settings dialog. Skills is a placeholder; Integrations manages owner-scoped GitHub/Jira keys and agent grants, users.list UI stops after 20, and the 5 GB storage limit is not enforced.
...
Bedrock is the first production model provider because the Runtime already uses ConverseStream. A small checked-in models.json carries five Bedrock profiles and Gemini 3.8 Flash, grouped by company. The five Bedrock base rate cards came from the AWS ListFoundationModelAgreementOffers API in eaap/us-east-1 on 2026-09-22; cost totals remain estimates because long-context tiers and non-model charges are not fully handled. Gemini is enabled only when CloudFormation creates an AgentCore Identity managed API-key provider from a NoEcho parameter. Runtime receives a workload access token from AgentCore, obtains the key from that one provider, and calls Google's streaming GenerateContent API; no Google key enters the browser, ZIP, DynamoDB or logs. IAM grants its own workload identity, token vault, provider and managed secret, not unrelated providers. Its prompt count includes cache reads and its generated output includes thinking tokens; the adapter normalizes both into the existing receipt. Gemini Code Interpreter is deliberately unavailable until its function-call bridge is live-tested. Google's introductory rates expire December 31, 2026. Do not promise “all models” across providers; the catalog is deliberately narrow and reviewed.
...
Use the checked-in exact profile IDs, review Runtime IAM for cross-region destinations and first-time third-party model access, and make capped paid turns in a disposable stack. All five produced initial successful smoke turns, but Opus 5.5, GPT-6 Sol and GPT-6 Luna later showed NOT_AVAILABLE and AccessDenied while Marketplace setup was pending. At the account owner's explicit request, their agreements were created with the CLI on 2026-09-23. All three subsequently reported AVAILABLE; an administrator's capped invocation activated them, then fresh production Runtime turns succeeded for each. This sequence matters: an available agreement and an early successful request alone did not prove the subscription was usable by the Runtime. Keep scripted echo as the only inference mock in routine tests.
...
### 5. Make Code Interpreter a real, connected workbench

Follow [the detailed tools design](TOOLS.md). One shared custom PUBLIC Code Interpreter, a managed Web Search Gateway target, and the built-in Browser now run through the same small Runtime model loop with separate per-agent grants. Use the interpreter's Python/Node/shell and preinstalled `requests`/`boto3` before adding adapter code or packages. Verify remaining background/file operations, shell/credential persistence, cleanup, and live latency. The Runtime remains the authorization and budget boundary; it exposes no cross-user shell or blanket administrator role. Search/Browser usage is observed but unpriced until AWS cost attribution is reconciled.

One user-owned GitHub/Jira connection slice is implemented: the UI accepts a key once, a single revisioned DynamoDB bundle stores that user's keys, agents store grant IDs, and the Runtime writes only selected values to a short-lived interpreter session. A disposable live story covered create/list/rotate/revoke, Python/Node/shell, accidental output redaction, and read-only GitHub /user; no key was returned by the connection API. Remaining: Jira Cloud auth and live identity call, cross-user tests, transport/log audit, connection health, optional git/gh installation, Web Search, Browser, and background tasks. Full-access generated code can still transform/exfiltrate a granted key; this grant is not attenuated by UI intent.

Gate: a user can connect, test, use, inspect, and revoke one integration without editing the stack; another user cannot discover or invoke that credential; revocation stops the session carrying it. Web Search preserves source citations; Browser and Code Interpreter sessions are stopped after use.

The current API-key record is one USER#sub/CREDENTIALS item with revision and connectionsJson, capped at 10 connections. It stores the actual key and metadata in the existing encrypted-at-rest DynamoDB table. Agent rows store only granted IDs. Cognito subject is the access boundary; AWS tags are inventory aids, not authorization. This minimizes resources but gives the Runtime/table-reader role access to every user's keys, a tradeoff to review before multi-tenant expansion. OAuth, account-shared credentials and customer cross-account roles need distinct flows; do not force them into this API-key bundle.
