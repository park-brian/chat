...
A person signs in at the static GitHub Pages app, chooses an approved agent, starts a conversation, and can revisit and continue it after switching agents, reloading, or signing in again. They can see measured request/tool usage and their budget period, with unpriced components honestly marked. An administrator can manage people, approve and verify regional models, and inspect per-person or combined usage. User-owned projects, skills, credentials, Gateway tools, and managed Browser execution extend the same Runtime/configuration shape.

The first administrator's path must be complete: deploy stack; create/invite an administrator; sign in; discover a Bedrock candidate; review rates; run an explicitly capped paid verification; approve it; create an agent; chat. A new member sees an approved agent or a clear create-agent path. Never equate “AWS listed it” with “this Runtime can invoke it,” “it supports ConverseStream/tools,” “its price is correct,” or “our app approved it.”

Current live evidence covers Cognito login, deterministic control mutations, scripted multi-turn Memory, Code Interpreter, usage queries, and one sticky Runtime session. It does **not** cover a paid production-model turn, all Cognito roles, chat reopening, accurate total AWS cost, Browser/Gateway/skills, or a populated-account visual pass. The production catalog is an administrator-maintained allowlist, not all Bedrock models. Worse, `models.put` currently rejects `:` and ARNs, while the read-only `eaap` us-east-1 catalog contains many active inference profiles with `:0`. The UI loses its displayed conversation on agent switch/reload. Skills and Integrations are placeholders, users.list UI stops after 20, and the 5 GB storage limit is not enforced.
...
4. **Model meaning.** Distinguish discovered AWS candidate, account-approved, recently verified, unavailable, and unpriced. Only approved, priced models are selectable in production. Verification is an explicit capped paid action recorded in usage. Approval cannot override AWS IAM, Region, Marketplace, or API restrictions.
...
| Project, agent, model approval/rates, limit overrides | One DynamoDB table through narrow Runtime commands | Revisioned rows, no separate deployment per agent |
...
| Models | `models.discover` reads regional foundation models/inference profiles as **candidates**; `models.put` accepts an exact valid Bedrock ID/ARN, explicit rate snapshot, inactive status and revision; `models.verify({id})` makes one capped, charged ConverseStream probe using the Runtime role; `models.setActive({id,revision,active})` requires verified capability and reviewed rates. |
...
Bedrock is the first production model provider because the Runtime already uses ConverseStream. A Gemini adapter is a later **provider-specific** option, not a reason to move ordinary inference or API keys into the browser. It must report Gemini's actual token/cache meters, have a separately reviewed price schedule and model capability catalog, and produce the same usage quality/terminal chat contract. Do not promise “all models” across providers; show only candidates and approved models with explicit provenance.
...
Fix model ID validation to accept exact API-supported IDs and ARNs while rejecting malformed/cross-region resources; do not simply allow arbitrary strings. Review Runtime IAM for cross-region profile destinations and first-time third-party model access. Use read-only `eaap` catalog calls for discovery, then make **one explicitly capped and priced paid turn** against an approved model in a disposable stack. Verify streaming, token metadata, optional tool schema, rate calculation, and a meaningful denial when Runtime lacks access. Keep scripted echo as the only inference mock in routine tests.

Gate: an administrator can enter a known valid regional model ID in the existing UI, and a member can send one real conversation; full in-app discovery/verification workflow is phase 2. The CLI catalog observation is not itself proof of invocation. AWS [model discovery](https://docs.aws.amazon.com/bedrock/latest/userguide/models-get-info.html), [ConverseStream](https://docs.aws.amazon.com/bedrock/latest/APIReference/API_runtime_ConverseStream.html), and [model access](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access.html) are the changing external contracts.

### 1. Make chat a chat client

Add conversation list/get and one navigation row per saved session; wire recent chats, explicit New chat, agent selection without destructive view reset, reopen after reload/login, pending/completed/failed turn state, terminal SSE validation, and safe code-block display. Include the Memory metering receipt and idempotent on-demand ledger/metadata reconciliation described above before presenting this as reliable saved chat. Preserve one sticky Runtime session per signed-in browser, but never confuse it with a durable Memory conversation session. The initial view must be useful with one agent and understandable with none.
...
### 2. Finish model onboarding and agent lifecycle

Add discover/verify/approve status, Region/capability information, price revisions/effective time, and deactivate/edit controls. Only future requests use new rates or a changed agent model; usage keeps the actual rate snapshot. Agent edit/archive uses revisions and retains historical conversation identity.

Gate: unsupported or denied models never become selectable; an inactive model blocks new turns while old conversation reads still work; a paid verification row appears in admin usage. Probe with the Runtime role rather than assuming the CLI administrator's access applies to it.
...
- **Normal first use:** No model is approved. An administrator discovers a candidate, sets rates, verifies with a bounded paid call, approves it, creates an agent, and chats. If any step requires a hand-written DynamoDB row or a model ID the API rejects, model onboarding (phases 0 and 2 together) is not done.
