...
Current live evidence covers Cognito login, deterministic control mutations, scripted multi-turn Memory, Code Interpreter, usage queries, one sticky Runtime session, the fixed model dropdown, capped Runtime turns for all five Bedrock models after Marketplace activation, a capped Gemini turn through AgentCore Identity, and desktop/mobile conversation reopen after switching and reload, revisioned agent edits, Code Interpreter grant/rotation/revocation, and a read-only real GitHub /user call. A JSON metering receipt is saved alongside each Memory turn. It does **not** cover all Cognito roles, cross-store reconciliation, accurate total AWS cost, Gemini managed-tool calls, Browser/Gateway/skills, or an exhaustive visual/accessibility pass. The production model list is checked-in JSON, not an admin table or settings dialog. Skills is a placeholder; Integrations now manages owner-scoped GitHub/Jira keys and agent grants, users.list UI stops after 20, and the 5 GB storage limit is not enforced.
...
Follow [the detailed tools design](TOOLS.md). Start with one shared custom PUBLIC Code Interpreter resource and isolated per-user/project/chat sessions. Use its existing Python/Node/shell/file/background-task operations and preinstalled `requests`/`boto3` before adding adapter code or packages. Verify on-demand installation, shell/credential persistence, session cleanup, and live latency. The Runtime remains the small model loop, authorization and budget boundary; it exposes no cross-user shell or blanket administrator role. Add the managed Web Search Gateway connector and built-in Browser separately, with their own grants and observed tool usage.
