...
Then ship one user-owned GitHub or Jira connection end to end: UI accepts a scoped credential once, Identity/vault stores it, the table holds only metadata, and the Runtime vends it to **that agent's explicitly granted interpreter session** after a secret-transport audit. The prompt gets variable names rather than secret values, but generated code can read the session-local `.env`. Full-access code can exfiltrate or exercise every permission of a granted key, so the grant UI must say this plainly; a read-only UI toggle cannot attenuate a write-capable PAT. If safe transport cannot be verified, leave credentialed code disabled rather than inventing a large provider-specific backend. Gateway targets are optional for integrations where their schema/policy materially helps, not a prerequisite for every Jira/GitHub call.
...
The minimal credential record is `{id,provider,authKind,scope,ownerSub?,projectId?,displayName,status,secretRef,createdAt,updatedAt}`. Scope is one of account, user, or user/project; table partition keys implement authorization/filtering, while AWS tags (`app`, `stack`, `owner-sub`, `project`, `integration-id`) support inventory and cleanup, never access control by themselves. The UI accepts a new/rotated secret once and never reads it back. Agent grants refer to credential IDs. A later Google Cloud integration should use the same scope/metadata contract with its own OAuth/service-account flow and an explicit code grant; it does not inherit unrelated secrets. For a customer's AWS account, prefer a scoped cross-account role and short-lived STS session over static keys; for S3 in our own stack, prefer the Runtime's scoped/presigned object flow.
