...
Current live evidence covers Cognito login, deterministic control mutations, scripted multi-turn Memory, Code Interpreter, usage queries, one sticky Runtime session, the fixed model dropdown, and capped real-model smoke turns. It does **not** cover durable third-party agreement access for every listed model, all Cognito roles, chat reopening, accurate total AWS cost, Gemini/Browser/Gateway/skills, or a full visual pass. The production model list is checked-in JSON, not an admin table or settings dialog. The UI loses its displayed conversation on agent switch/reload. Skills and Integrations are placeholders, users.list UI stops after 20, and the 5 GB storage limit is not enforced.
